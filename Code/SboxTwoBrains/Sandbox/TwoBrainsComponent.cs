...
public sealed class TwoBrainsComponent : Component
{
	...
	public string DebugStatusLine
	{
		get
		{
			...
			foreach ( var kv in a.PendingMeta )
				pending.Add( kv.Value.Kind.ToString() );
			var pendingKinds = pending.Count == 0 ? "-" : string.Join( "+", pending );
			var flags = a.Flags.Count == 0 ? "-" : string.Join( ",", a.Flags );
